    ...
    <aside class="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 shadow-sm z-20">
      ...
      <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        <UButton
          ...
          :class="route.path === localePath(item.path) ? 'bg-blue-50 text-blue-700 font-bold' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'"
        >
          <template #leading>
      ...
      <div class="p-4 border-t border-slate-200">
        <div class="flex items-center mb-4 px-2">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-slate-500 truncate">{{ authStore.isAdmin ? 'Администратор (321)' : 'Оператор' }}</p>
          </div>
        </div>
        <UButton
        ...
        <div class="flex flex-col h-full bg-white">
          ...
          <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
            <UButton
              ...
              :class="route.path === localePath(item.path) ? 'bg-blue-50 text-blue-700 font-bold' : 'text-slate-600'"
...
const allNavItems = [
  ...
  { name: 'Модерация товаров', path: '/admin/products/moderation', icon: 'i-lucide-shield-alert', roles: ['admin'] },
  { name: 'Заявки поставщиков', path: '/admin/suppliers', icon: 'i-lucide-file-text', roles: ['admin'] },
  { name: 'Каталог товаров', path: '/admin/products', icon: 'i-lucide-package', roles: ['admin', 'operator'] },
  { name: 'Категории', path: '/admin/categories', icon: 'i-lucide-folder-tree', roles: ['admin', 'operator'] },
  { name: 'Заказы клиентов', path: '/admin/orders', icon: 'i-lucide-shopping-bag', roles: ['admin', 'operator'] },
...
const breadcrumbItems = computed(() => {
  ...
  if (route.path.includes('/products/moderation')) {
    items.push({ label: 'Модерация товаров', to: localePath('/admin/products/moderation') })
  } else if (route.path.includes('/suppliers')) {
    items.push({ label: 'Заявки поставщиков', to: localePath('/admin/suppliers') })
